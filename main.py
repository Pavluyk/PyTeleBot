...
import menuBot
from menuBot import Menu  # в этом модуле есть код, создающий экземпляры классов описывающих меню
import DZ  # домашнее задание c первого урока
import fun  # развлечения
import speech  # работа с речью
import ffmpeg

bot = telebot.TeleBot("5464097686:AAFtNwPjlQwVHFodSAAzmmecrpsRk4v7UKE")  # Создаем экземпляр бота
...
def send_help(bot, chat_id):
    bot.send_message(chat_id, "Автор: Мария Павлюк")
    markup = types.InlineKeyboardMarkup()
    btn1 = types.InlineKeyboardButton(text="Напишите автору", url="https://t.me/Pavlyuk_Maria")
    markup.add(btn1)
